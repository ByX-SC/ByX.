...
function updateStickmanESP() 
    for player, esp in pairs(espObjects) do 
        ...
        if espSettings.ShowDistance and anyVisible then 
...
local function enableStickmanESP() 
    for _, player in pairs(players:GetPlayers()) do 
...
local function disableStickmanESP() 
    if connections.stickman then 
        connections.stickman:Disconnect() 
        connections.stickman = nil 
    end 
...
VisualsTab:CreateToggle({ 
    ...
    Callback = function(Value) 
        ESPEnabled = Value 
        if not Value then 
            -- Disable all other ESPs when main ESP is disabled 
            ShowHealth = false 
            ShowInventory = false 
...
VisualsTab:CreateToggle({ 
    ...
    Callback = function(Value) 
        xrayEnabled = Value 
        -- Implement Xray logic if needed 
        RefreshAllESP() 
    end 
}) 
...
local function GetPredictedPosition(targetPart)  
    ...
    if PredictionEnabled then  
        local timeToHit = (Camera.CFrame.Position - targetPart.Position).Magnitude / BulletSpeed  
